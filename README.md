# Clases-Python

<div align="left">
  Clase Python 1
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%201.py" target="_blank">
    <button>[Introducción a Python]</button>
  </a>
  <br>
  Clase Python 2
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%202.py" target="_blank">
    <button>[Continuación]</button>
  </a>
  <br>
  Clase Python 3
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%203.py" target="_blank">
    <button>[Estructuras de Control]</button>
  </a>
  <br>
  Clase Python 4
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%204.py" target="_blank">
    <button>[Funciones y Métodos]</button>
  </a>
  <br>
  Clase Python 5
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%205.py" target="_blank">
    <button>[Listas y Diccionarios]</button>
  </a>
  <br>
  Clase Python 6
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%206.py" target="_blank">
    <button>[POO Básico]</button>
  </a>
  <br>
  Clase Python 7
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%207.py" target="_blank">
    <button>[Herencia y Más OOP]</button>
  </a>
  <br>
  Clase Python 8
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%208.py" target="_blank">
    <button>[POO avanzada y objetos]</button>
  </a>
  <br>
  Clase Python 9
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%209.py" target="_blank">
    <button>[Herencia y Polimorfismo]</button>
  </a>
  <br>
  Clase Python 10
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%2010.py" target="_blank">
    <button>[Excepciones]</button>
  </a>
  <br>
  Clase Python 11
  <a href="https://github.com/EnriqueBDeL/Clases-Python/blob/main/Clase%2011.py" target="_blank">
    <button>[Archivos .txt]</button>
  </a>
  <br>
</div>

<br><br>

> ⚠️ **IMPORTANTE**  
> Este repositorio ha sido creado durante mis estudios, por lo que puede contener errores o no seguir todas las mejores prácticas de programación. ¡Utilízalo como apoyo pero contrasta siempre la información!

<br><br>

## 🌎 ¡Sígueme en mis redes y colaboremos!

Si te interesa el contenido que comparto, te invito a seguirme en mis redes para estar al tanto de las últimas actualizaciones, proyectos y más.

<p align="left">
  <a href="https://enriquebdl.taplink.ws/" target="_blank">
    <img src="https://images.g2crowd.com/uploads/product/image/social_landscape/social_landscape_dba52b0470340e16dbb5612c34cce7ad/taplink.png" 
         alt="Taplink" 
         height="30" 
         width="60" />
  </a>
</p>
